...
void cliShowList(cli_args_t *args)
{
    ...
    cli_t *p_cli = &cli_node;
    /*
    uartMsgByte2ASC(p_cli->ch,"p_cli->cmd_args.argc = ",p_cli->cmd_args.argc);
    delay_ms(50);
    //
    if(p_cli->cmd_args.argc != 0x00)
    {
        for(i= 1 ; i <= p_cli->cmd_args.argc ; i++)
        {
            uartMsg(p_cli->ch,"\r\np_cli->cmd_args.argv = ");
            delay_ms(50);
            uartMsg(p_cli->ch,p_cli->argv[i]);
            delay_ms(50);
        }
    }
    */
