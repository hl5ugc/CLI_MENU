...
// Global enable interrupts
    ...
    Cli_CMD_Open();
    //
    cliAdd("MM"," mm a3a000 1", cb_Cli_MM);
    cliAdd("MD"," md a01000 1", cb_Cli_MD);
...
static void cb_Cli_MM(cli_args_t *args)
{
    ...
    if(args->argc >= 1) {
        uartMsg(_DEF_UART1,"\n\rargs->argv[0]");
        delay_ms(10) ;
        uartMsg(_DEF_UART1," => 0x") ;
        delay_ms(10) ;
        uartMsg(_DEF_UART1,args->argv[0]) ;
        delay_ms(10) ;
        //
        if(args->argc >= 2) {
            uartMsg(_DEF_UART1,"\n\rargs->argv[1]");
            ...
            uartMsg(_DEF_UART1," => 0x") ;
            delay_ms(10) ;
            uartMsg(_DEF_UART1,args->argv[1]) ;
            delay_ms(10) ;
        //
            if(args->argc >= 3) {
                uartMsg(_DEF_UART1,"\n\rargs->argv[2]");
                ...
                uartMsg(_DEF_UART1," => 0x") ;
                delay_ms(10) ;
                uartMsg(_DEF_UART1,args->argv[2]) ;
                delay_ms(10) ;
            }
        }
    }
    u32Address = args->get32Data(0) ;
    //addr = (unsigned int *)u32Address ;
    //u16Indata = *addr ;
    uartMsg(_DEF_UART1,"\n\Address => 0x") ;

    uartWord4Hex(_DEF_UART1 ,u32Address>>16) ;
    uartWord4Hex(_DEF_UART1 , u32Address) ;
    //
    u16Indata = args->getData(1) ;
    uartMsg(_DEF_UART1,"\n\Data => 0x") ;
    uartWord4Hex(_DEF_UART1 , u16Indata) ;
}
static void cb_Cli_MD(cli_args_t *args)
{
    uint16_t u16Indata = 0 ;
